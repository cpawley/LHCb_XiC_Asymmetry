...
def cut_tree(label, path, extension, jobs, filter_str, variables, tree_name):
	...
	out_file = ROOT.TFile(f'/data/bfys/mkoopmans/data/{label}_cut.root', 'recreate')
...
def cut_tree_small_files(label, path, extension, jobs, filter_str, variables, tree_name):
	...
	print(f'Succesfully cut and written {label} data to output files')
...
def main():
	...
	cut_tree_small_files('background', bkg_file, 'Lc2pKpiTuple.root', bkg_subjobs, bkg_filter_str, cut_variables, tree_name)
